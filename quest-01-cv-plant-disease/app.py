...
Single-page layout:
  ...
  - Live Inference (sample images + upload + predictions)
  - Confusion Matrix, Per-Class Performance, Confusion Analysis
  - Grad-CAM Visualisations, Training Configuration
"""
...
def get_sample_images():
    """Get sorted list of sample image paths from the sample directory."""
    sample_dir = HERE / "data" / "sample"
    if not sample_dir.exists():
        return []
    return sorted(sample_dir.iterdir())
...
st.markdown(
    "Upload a leaf image **or choose a sample below** to identify diseases "
    "and healthy conditions across **15 classes** covering 3 crop species."
)
...
if session is None:
    ...
    # ── Session state for sample selection ─────────────────────────────
    ...
    if sample_images:
        with st.expander("📁 Sample Images — click to test", expanded=True):
            st.markdown("Choose a pre-loaded leaf image to quickly test the model:")
            for i in range(0, len(sample_images), 8):
                cols = st.columns(8)
                for col, img_path in zip(cols, sample_images[i : i + 8]):
                    with col:
    ...
    if uploaded_file is not None:
        ...
        image_caption = f"Sample: {Path(sample_path).stem.replace('_', ' ').title()}"

    if image_to_process is not None:
        col1, col2 = st.columns([1, 1])

        with col1:
            st.image(image_to_process, caption=image_caption, width="stretch")

        with col2:
            with st.spinner("Analysing leaf..."):
                results, probs = predict(image_to_process, session, class_names)
        ...
        st.info("👆 Upload a leaf image or click a sample above to get started.")
        st.markdown("""
...
if metrics is not None:
    ...
    if report_lines:
        ...
        classes_display = [r["class"].replace("_", " ").title() for r in report_sorted]
...
if gradcam_images:
    ...
    st.info(
        "No Grad-CAM images found. Run `python src/evaluate.py --gradcam-samples 8`."
    )
...
# ── Footer ─────────────────────────────────────────────────────────────────
st.divider()
st.caption("Built with MobileNetV2 | PlantVillage Dataset")
